...
import { useAssignEvents } from "../hooks/useAssignEvents";

export default function AssignEventPage() {
  const { id } = useParams();
...
useEffect(() => {
  if (admin?.managed_events) {
    setSelectedEvents(
      admin.managed_events.map(
        (event) => event.id
      )
    );
  }
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-500">
              Pilih event yang bisa dikelola oleh admin ini
            </p>
          </div>
        </div>
      </div>
